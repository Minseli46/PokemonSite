/**
 * Outils spécifiques au Quiz Agent
...
export const quizToolDefinitions: ToolDefinition[] = [
  {
    type: 'function',
    function: {
      ...
      parameters: {
        ...
        properties: {
          mode: {
            type: 'string',
            description: 'Le mode de quiz',
            enum: ['name', 'type', 'stat', 'evolution', 'ability', 'generation'],
          },
          difficulty: {
            type: 'string',
            description: 'La difficulté. easy = Gen 1 (1-151), medium = Gen 1-4 (1-493), hard = toutes (1-1025)',
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          generation_id: {
            type: 'string',
            description: 'Le numéro de la génération (1-9)',
          },
        },
...
async function generateQuizQuestion(args: Record<string, any>): Promise<string> {
  const { mode = 'name', difficulty = 'easy' } = args;
  ...
  try {
    ...
    switch (mode) {
      ...
      case 'stat': {
        ...
        const wrongStats = pokemon.stats
          .filter((s: any) => s.stat.name !== correctAnswer)
          .map((s: any) => s.stat.name)
          .slice(0, 3);
      ...
      case 'ability': {
        ...
        // Récupérer des abilities aléatoires d'autres Pokémon
      ...
      default: { // name
        // Récupérer 3 autres Pokémon aléatoires pour les mauvaises réponses
...
async function getPokemonTrivia(args: Record<string, any>): Promise<string> {
  try {
    const { pokemon_name } = args;
...
async function getGenerationInfo(args: Record<string, any>): Promise<string> {
  try {
    ...
    return JSON.stringify({ error: `Génération "${args.generation_id}" non trouvée` });
  }
}
...
export const quizToolExecutors: ToolRegistry = {
  generate_quiz_question: generateQuizQuestion,
  get_pokemon_trivia: getPokemonTrivia,
  get_generation_info: getGenerationInfo,
};
